...
async function ensureImportantTasksTable() {
  ...
  `
  await sql`ALTER TABLE important_tasks ADD COLUMN IF NOT EXISTS resource_url TEXT`
}
...
export async function createImportantTask(
  ...
  const result = await sql<ImportantTask[]>`
    INSERT INTO important_tasks (text, numerator, denominator, days_remaining, resource_url)
    VALUES (
      ${data.text ?? ""},
      ${data.numerator ?? 0},
      ${data.denominator ?? 1},
